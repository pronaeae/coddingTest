...
rl.on("line", function (line) {
  ...
  const [option, ...X] = inputData;
  const [N, C] = option.split(" ").map(Number);

  const houses = X.map(Number).sort((a, b) => a - b);
  ...
  console.log(binarySearch(houses, C));
});